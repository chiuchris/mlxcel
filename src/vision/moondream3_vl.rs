...
//! Moondream3 vision-language wrapper.
//!
//! Moondream3 prepends a BOS token plus projected image tokens instead of
//! inserting image placeholder IDs into the text stream. The wrapper therefore
//! assembles one combined embedding buffer and a custom prefix-friendly mask.
...
pub(crate) fn build_moondream3_attention_mask(
    prefix_tokens: usize,
    prompt_tokens: usize,
) -> UniquePtr<MlxArray> {
...
impl LanguageModel for Moondream3VLModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
