...
pub struct Gemma3StageExecutor {
    ...
    cache_sets: RefCell<HashMap<usize, Vec<Gemma3Cache>>>,
}
...
impl Gemma3StageExecutor {
    ...
    fn sequence_needs_reset(external_caches: &[KVCache], internal_caches: &[Gemma3Cache]) -> bool {
        external_caches.iter().all(|cache| cache.offset == 0)
            && internal_caches.iter().any(|cache| cache.offset() > 0)
    }

    fn sync_external_offsets(external_caches: &mut [KVCache], internal_caches: &[Gemma3Cache]) {
