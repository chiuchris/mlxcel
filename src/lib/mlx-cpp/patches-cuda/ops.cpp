...
//
// Modified from upstream MLX v0.31.1 mlx/ops.cpp
//
// Changes:
//   - Added bf16_mixed_astype() helper that skips astype conversion when the
//     CUDA mixed-precision binary kernel can handle the type mismatch directly.
...
// CUDA patch: conditionally apply astype for mixed-precision binary ops.
// When out_type is bf16 and the input is fp32, skip the conversion -- the
// mixed-precision kernel in binary.cuh will handle it directly, avoiding a
// separate copy_v kernel launch.
array bf16_mixed_astype(
    const array& x,
    const Dtype& out_type,
    StreamOrDevice s) {
  if (out_type == bfloat16 && x.dtype() == float32) {
    return x; // Let the mixed-precision kernel handle fp32 input directly
  }
  return astype(x, out_type, s);
}
...
/** Reduce-mode slice_update (internal helper) */
array slice_update(
    const array& src,
    const array& update,
    Shape start,
...
array add(const array& a, const array& b, StreamOrDevice s /* = {} */) {
  ...
  auto inputs = broadcast_arrays(
      {bf16_mixed_astype(a, out_type, s),
       bf16_mixed_astype(b, out_type, s)},
      s);
  auto& shape = inputs[0].shape();
  return array(
...
array divide(const array& a, const array& b, StreamOrDevice s /* = {} */) {
  ...
  auto inputs = broadcast_arrays(
      {bf16_mixed_astype(a, dtype, s),
       bf16_mixed_astype(b, dtype, to_stream(s))},
      s);
  auto& shape = inputs[0].shape();
  return array(
