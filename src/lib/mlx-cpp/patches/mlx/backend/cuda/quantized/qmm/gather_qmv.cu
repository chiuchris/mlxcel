...
// Modified by mlxcel: GatherQMM CUDA implementation for MoE index-based
// quantized matrix-vector multiply. Based on qmv.cu with index array support.
...
gather_dequant_fma(const T* x, const Q* w, S scale, T bias, T* out) {
...
__device__ __forceinline__ void
gather_dequant_fma(const T* x, const Q* w, S scale, T bias, float* out) {
  // Read x/w into registers.
  auto x_vec = *(reinterpret_cast<const cutlass::Array<T, N>*>(x));
  auto w_vec = *(reinterpret_cast<const cutlass::Array<Q, N>*>(w));
  // Output is assumed to be registers.
...
// GatherQMM variant of qmv_kernel:
// Uses lhs_indices/rhs_indices to select which token and expert weights
// for each batch element, enabling MoE (Mixture of Experts) routing.
template <
    int rows_per_block,
...
__global__ void gather_qmv_kernel(
  ...
  // Gather: use index arrays instead of direct batch indexing.
  int batch_idx = block.group_index().z;
  int x_batch = lhs_indices[batch_idx];
  int w_batch = rhs_indices[batch_idx];

  // Advance pointers of x/out using gathered indices.
  // x uses lhs_indices to select which token.
  x += block.group_index().y * k + m * k * x_batch;
  // out uses sequential batch_idx for output placement.
  out += block.group_index().y * n + m * n * batch_idx;
  ...
  // Advance w/scales/biases to current row using rhs_indices for expert
  // selection.
  w += (static_cast<int64_t>(row) + static_cast<int64_t>(n) * w_batch) *
      w_step(k);
  scales += (static_cast<int64_t>(row) + static_cast<int64_t>(n) * w_batch) *
      groups_per_row;
  if constexpr (has_bias) {
    biases +=
        (static_cast<int64_t>(row) + static_cast<int64_t>(n) * w_batch) *
        groups_per_row;
  }
  ...
  auto dequant_fma_tile = [&](int idx) {
    ...
    gather_dequant_fma<elems_per_thread, has_bias>(
...
template <typename F>
inline void
gather_dispatch_element_types(Dtype dtype, const char* tag, F&& f) {
  if (dtype == float32) {
...
template <typename F>
inline void gather_dispatch_groups(int group_size, const char* tag, F&& f) {
  if (group_size == 32) {
...
inline void gather_dispatch_quant_types(
    int bits,
    int group_size,
  ...
  if (mode == QuantizationMode::Mxfp4) {
    ...
    gather_dispatch_groups(group_size, tag, [&]<int group_size>() {
      if (bits == 2) {
        ...
        throw std::invalid_argument(fmt::format(
            "{} {}-bit quantization is not supported.", tag, bits));
      }
    });
  }
}
...
void gather_qmv(
  ...
  const char* tag = "[gather_qmv]";
  int m = out.shape(-2);
  int n = out.shape(-1);
  int k = x.shape(-1);
  int l = out.size() / (m * n);

  gather_dispatch_element_types(out.dtype(), tag, [&]<typename T>() {
    gather_dispatch_quant_types<T>(
        bits,
        group_size,
        mode,
        tag,
        [&]<typename Q, typename S, int group_size>() {
          ...
          cu::gather_qmv_impl<group_size, has_bias>(
              gpu_ptr<T>(x),
              gpu_ptr<Q>(w),
              gpu_ptr<S>(scales),
