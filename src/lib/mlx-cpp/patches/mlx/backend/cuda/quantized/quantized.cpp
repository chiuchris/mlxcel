...
// Modified by mlxcel:
//   - Added NVTX annotations for QMM kernel dispatch path tracing (Phase 2a)
//   - Added GatherQMM::eval_gpu for CUDA-native MoE support
...
void QuantizedMatmul::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  auto call_qmm_sm90 = [&]() {
    nvtx3::scoped_range kr("QMM_dispatch:qmm_sm90");
    out.set_data(cu::malloc_async(out.nbytes(), encoder));
    qmm_sm90(x, w, scales, *biases, out, bits_, group_size_, encoder, s);
  };
  auto call_fp_qmv = [&]() {
    nvtx3::scoped_range kr("QMM_dispatch:fp_qmv");
    out.set_data(cu::malloc_async(out.nbytes(), encoder));
    fp_qmv(x, w, scales, out, bits_, group_size_, encoder, s);
  };
  auto call_qmv = [&]() {
    nvtx3::scoped_range kr("QMM_dispatch:qmv");
    out.set_data(cu::malloc_async(out.nbytes(), encoder));
...
void GatherQMM::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  // Parse inputs: x, w, scales, [biases], lhs_indices, rhs_indices
  const array& x = inputs[0];
  const array& w = inputs[1];
  const array& scales = inputs[2];
  std::optional<array> biases;
  ...
  const array& lhs_indices = inputs[idx++];
  const array& rhs_indices = inputs[idx++];

  // Ensure row-contiguous inputs.
  array x_contig = ensure_row_contiguous(x, encoder, s);
  array w_contig = ensure_row_contiguous(w, encoder, s);
  array scales_contig = ensure_row_contiguous(scales, encoder, s);
  std::optional<array> biases_contig;
  if (biases) {
    biases_contig = ensure_row_contiguous(*biases, encoder, s);
  }
  array lhs_contig = ensure_row_contiguous(lhs_indices, encoder, s);
  array rhs_contig = ensure_row_contiguous(rhs_indices, encoder, s);

  // Allocate output.
  out.set_data(cu::malloc_async(out.nbytes(), encoder));

  // Call the gather_qmv kernel.
  gather_qmv(
      x_contig,
      w_contig,
      scales_contig,
