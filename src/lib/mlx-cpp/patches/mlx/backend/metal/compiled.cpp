...
inline void build_kernel(
  ...
  for (auto& x : tape) {
    ...
    if (is_static_cast(x.primitive())) {
      ...
      auto out_type = get_type_string(x.dtype());
      os += x.primitive().name();
      os += "()(";
      for (int i = 0; i < x.inputs().size() - 1; i++) {
        auto in_type = get_type_string(x.inputs()[i].dtype());
        if (in_type != out_type) {
          os += fmt::format(
              "static_cast<{0}>(tmp_{1}), ",
              out_type,
              namer.get_name(x.inputs()[i]));
        } else {
          os += fmt::format("tmp_{0}, ", namer.get_name(x.inputs()[i]));
        }
      }
      auto& last = x.inputs().back();
      auto last_type = get_type_string(last.dtype());
      if (last_type != out_type) {
        os += fmt::format(
            "static_cast<{0}>(tmp_{1}));\n",
            out_type,
            namer.get_name(last));
