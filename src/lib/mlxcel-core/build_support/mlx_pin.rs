...
/// The MLX upstream repository marker.
pub const MLX_REPOSITORY_MARKER: &str = "ml-explore/mlx";

/// The fork mirror used by the local mlxcel build.
pub const MLX_FORK_REPOSITORY_MARKER: &str = "chiuchris/mlx";

/// Repository markers accepted for the single pinned MLX declaration.
const MLX_REPOSITORY_NAMES: &str = "ml-explore/mlx or chiuchris/mlx";
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoMlxDeclaration { path } => write!(
                f,
                "cannot resolve the pinned MLX commit: {} has no FetchContent_Declare block whose GIT_REPOSITORY names {MLX_REPOSITORY_NAMES}. The pin is the GIT_TAG argument of that block and nothing else stores it.",
                path.display()
            ),
            Self::AmbiguousMlxDeclaration { path, count } => write!(
                f,
                "cannot resolve the pinned MLX commit: {} has {count} FetchContent_Declare blocks naming {MLX_REPOSITORY_NAMES}, so the pin is ambiguous. Leave exactly one.",
                path.display()
            ),
            Self::MissingGitTag { path } => write!(
                f,
                "cannot resolve the pinned MLX commit: the FetchContent_Declare block naming {MLX_REPOSITORY_NAMES} in {} has no {GIT_TAG_KEYWORD} argument.",
                path.display()
            ),
            Self::AmbiguousGitTag { path, count } => write!(
                f,
                "cannot resolve the pinned MLX commit: the FetchContent_Declare block naming {MLX_REPOSITORY_NAMES} in {} has {count} {GIT_TAG_KEYWORD} arguments. Leave exactly one.",
                path.display()
            ),
...
mod tests {
    ...
    #[test]
    fn parses_the_pin_from_the_fork_mirror_declaration() {
        let source = format!(
            "FetchContent_Declare(\n  mlx\n  GIT_REPOSITORY \"https://github.com/chiuchris/mlx.git\"\n  GIT_TAG {OTHER_PIN})\n"
        );
        assert_eq!(parse_pinned_commit(&source, &path()).unwrap(), OTHER_PIN);
    }
