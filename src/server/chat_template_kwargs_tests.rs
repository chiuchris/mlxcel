...
// Env tests share a process-wide variable — serialize through a SINGLE
// shared mutex across every env-touching test in this file. Using per-test
// mutexes would let cargo's parallel runner interleave env mutations across
// tests and race on the shared `LLAMA_ARG_CHAT_TEMPLATE_KWARGS` slot, causing
// spurious failures that are hard to reproduce locally.
