...
//! ERNIE 4.5 model implementation using mlxcel-core
//!
//! Baidu's ERNIE 4.5 - Standard Llama architecture with optional bias support
//! Key differences from Llama:
//! - Optional bias in attention projections (controlled by use_bias config)
//! - RoPE with traditional=true
...
impl Attention {
    pub fn forward(
        ...
        // Apply RoPE with traditional=true for ERNIE
        let q = mlxcel_core::fast_rope(
            &q,
            self.rope_dims,
            true, // traditional=true for ERNIE
            self.rope_base,
            1.0, // scale
            offset,
        );
...
impl LanguageModel for Ernie45Model {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
        ...
        Ernie45Model::forward(self, input_ids, caches, mask)
    }

    fn make_caches(&self) -> Vec<KVCache> {
        Ernie45Model::make_caches(self)
    }
