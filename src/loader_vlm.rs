...
///
/// LLaVA text_config may be minimal. We can infer:
...
fn infer_llama_config_from_weights(
    ...
    // Infer hidden_size from model.norm.weight
        ...
        && let Some(w) = weights.get("model.norm.weight")
    {
        ...
        if !shape.is_empty() {
            obj.insert(
                ...
                serde_json::Value::Number(serde_json::Number::from(shape[0] as u64)),
            );
        }
    }

    // Infer num_hidden_layers from counting layer keys
    if obj
        .get("num_hidden_layers")
        .and_then(|v| v.as_u64())
        .is_none()
    {
        ...
        obj.insert(
            ...
            serde_json::Value::Number(serde_json::Number::from(max_layer + 1)),
        );
    }

    // Infer intermediate_size from gate_proj weight
    if obj
        .get("intermediate_size")
        .and_then(|v| v.as_u64())
        .is_none()
    {
        // Try quantized first (has .scales), then dense
    ...
    // Infer num_attention_heads from hidden_size (default head_dim=128)
    if obj
        .get("num_attention_heads")
        .and_then(|v| v.as_u64())
        .is_none()
    ...
    {
        let num_heads = hidden / 128; // standard head_dim for Llama
        obj.insert(
...
pub(crate) fn load_llava_vlm(model_path: &Path) -> Result<LoadedModel> {
    ...
    // Detect text model backend from text_config.model_type
    ...
    // LLaVA text_config may be minimal (missing hidden_size, num_hidden_layers, etc.)
    // Infer missing fields from the weight shapes
    ...
    // Build text model based on backend type
        ...
        "llama" | "mistral" => {
            let text_args: models::llama3::ModelArgs =
                serde_json::from_value(text_config_value.clone())
                    .map_err(|e| anyhow::anyhow!("Failed to parse text_config as Llama: {}", e))?;
            let m = models::Llama3Model::from_weights(&weights, &text_args)
                .map_err(|e| anyhow::anyhow!("Failed to load text model: {}", e))?;
            LoadedModel::Llama(m)
        }
        "qwen2" => {
            // Qwen2Model is an alias for Llama3Model
            let text_args: models::llama3::ModelArgs =
                serde_json::from_value(text_config_value.clone())
                    .map_err(|e| anyhow::anyhow!("Failed to parse text_config as Qwen2: {}", e))?;
            let m = models::Qwen2Model::from_weights(&weights, &text_args)
                .map_err(|e| anyhow::anyhow!("Failed to load text model: {}", e))?;
            LoadedModel::Qwen2(m)
        }
        _ => {
            return Err(anyhow::anyhow!(
                "Unsupported LLaVA text backend: {}",
                text_model_type
            ));
        }
    };

    // Get quantization params from top-level config
    ...
    // Build vision encoder (SigLIP or CLIP - both handled by SigLipVisionModel)
    ...
    // Build image processor
    // CLIP uses different normalization than SigLIP
    let vision_model_type = vlm_config.vision_config.model_type.as_str();
    let processor = if vision_model_type.contains("clip") {
        SigLipProcessor {
            image_size: vlm_config.vision_config.image_size,
            mean: [0.48145466, 0.4578275, 0.40821073], // CLIP normalization
            std: [0.26862954, 0.261_302_6, 0.275_777_1],
            do_normalize: true,
        }
    } else {
        SigLipProcessor::new(vlm_config.vision_config.image_size)
    };
    ...
    let vision_module = vision::VisionModule {
        ...
        boi_token_id: 0, // LLaVA doesn't use BOI/EOI framing
        eoi_token_id: 0,
        mm_tokens_per_image,
...
/// Load a LLaVA-Bunny VLM model (SigLIP + MLP projector + Qwen2 text)
///
/// Very similar to LLaVA but with different weight key conventions:
/// - vision_tower.vision_tower.vision_model.* (extra nesting)
/// - mm_projector.* instead of multi_modal_projector.*
pub(crate) fn load_llava_bunny_vlm(model_path: &Path) -> Result<LoadedModel> {
    use vision::connectors::mlp::MLPProjector;
    use vision::encoders::siglip::SigLipVisionModel;
    use vision::processors::siglip::SigLipProcessor;

    // Parse config as Value (Bunny configs have sparse vision_config)
    let config_path = model_path.join("config.json");
    let config_str = std::fs::read_to_string(&config_path)
        .map_err(|e| anyhow::anyhow!("Failed to read config.json: {}", e))?;
    let config_str = sanitize_config_json(&config_str);
    ...
    // Remap weight keys: strip "language_model." prefix, then sanitize bunny-specific keys
    let mut weights = mlxcel_core::weights::WeightMap::new();
    for (key, value) in raw_weights {
        let new_key = if let Some(stripped) = key.strip_prefix("language_model.") {
            stripped.to_string()
    ...
    // Sanitize LLaVA-Bunny weight keys:
    // - vision_tower.vision_tower.vision_model.* → vision_tower.vision_model.*
    // - mm_projector.* → multi_modal_projector.*
    // - model.lm_head.* → lm_head.* (Bunny wraps lm_head under model.)
    let keys: Vec<String> = weights.keys().cloned().collect();
    for key in keys {
        let new_key = if let Some(rest) = key.strip_prefix("vision_tower.vision_tower.") {
    ...
    // Detect text model backend from text_config or top-level config
    // Bunny configs are flat (text config at top level), so fall back to top-level fields
    let text_config_val = full_config.get("text_config").cloned().unwrap_or_default();
    let text_model_type = text_config_val
        .get("model_type")
        .and_then(|v| v.as_str())
        .or_else(|| {
            // For Bunny: detect from top-level model_type or language_model
            let mt = full_config.get("model_type").and_then(|v| v.as_str())?;
            if mt.contains("llama") {
                Some("llama")
            } else if mt.contains("qwen") {
                Some("qwen2")
            } else {
                None
            }
    ...
    // For Bunny: text config may be at top level (not nested under text_config)
        ...
        && text_config_val.as_object().map(|o| o.len()).unwrap_or(0) > 0
    {
        ...
        // Use top-level config as text config
        full_config.clone()
    };
    infer_llama_config_from_weights(&mut text_config_value, &weights);
    ...
    let text_model: LoadedModel = match text_model_type {
        ...
        _ => {
            return Err(anyhow::anyhow!(
                "Unsupported LLaVA-Bunny text backend: {}",
                text_model_type
            ));
        }
    };
    ...
    // Build vision config — try parsing from config, fall back to SigLIP defaults
    ...
    let vision_config: vision::config::VisionConfig =
        serde_json::from_value(vision_config_val.clone()).unwrap_or_else(|_| {
            // Construct from mm_vision_tower hint and known SigLIP defaults
    ...
    // Build image processor (SigLIP with CLIP normalization for CLIP-based models)
    let mm_vision_tower = full_config
        .get("mm_vision_tower")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let processor = if mm_vision_tower.contains("clip") && !mm_vision_tower.contains("siglip") {
        SigLipProcessor {
        ...
        SigLipProcessor::new(vision_config.image_size)
    };
...
pub(crate) fn load_qwen2_5_vl(model_path: &Path) -> Result<LoadedModel> {
    ...
    // Build text model (same Qwen2VL language model)
    let text_model = models::Qwen2VLModel::from_weights(&weights, &text_config)
        .map_err(|e| anyhow::anyhow!("Failed to load Qwen2.5VL text model: {}", e))?;

    // Build vision encoder (new Qwen2.5-VL encoder)
    let vision_encoder =
...
pub(crate) fn load_qwen3_vl_moe(model_path: &Path) -> Result<LoadedModel> {
    ...
    // Build vision encoder (same as Qwen3-VL)
    let vision_encoder =
        Qwen3VLVisionEncoder::from_weights(&weights, &vision_config, "vision_tower")
...
pub(crate) fn load_qwen3_5_vlm(model_path: &Path) -> Result<LoadedModel> {
    ...
    // "model.visual.xxx" -> "vision_tower.xxx"
    // "language_model.xxx" -> "xxx" (already-sanitized format)
    let mut text_weights = mlxcel_core::weights::WeightMap::new();
    ...
    for (key, value) in raw_weights {
        if key.starts_with("model.language_model.") || key.starts_with("language_model.model.") {
            ...
            // Direct model weights (text model)
            text_weights.insert(key, value);
        }
    }

    // Parse text config and build text model
    let mut text_config_val = full_config
        .get("text_config")
        .cloned()
        .ok_or_else(|| anyhow::anyhow!("Missing text_config in config.json"))?;

    // Merge quantization from top level if needed
    ...
    // Sanitize text weights (handles norm shift, conv1d transpose, MoE stacking)
    ...
    // Set MRoPE on full attention layers for VLM support
