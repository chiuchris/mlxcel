...
if [ -z "$VERSION" ] || [ -z "$DISTRO" ] || [ -z "$PPA" ]; then
    ...
    echo "Example: $0 0.0.6 noble lablup/backend-ai"
    exit 1
fi
...
get_highest_revision() {
    ...
    if command -v rmadison >/dev/null 2>&1; then
        ...
        existing_versions=$(rmadison -s "$distro" -a source mlxcel 2>/dev/null | grep -E "${base_version}-[0-9]+~${distro}[0-9]+" | awk '{print $2}' || true)
    else
        ...
        api_url="https://api.launchpad.net/1.0/~${ppa_owner}/+archive/ubuntu/${ppa_name}?ws.op=getPublishedSources&source_name=mlxcel&distro_series=https://api.launchpad.net/1.0/ubuntu/${distro}&status=Published"
    ...
    for ver in $existing_versions; do
        # Extract revision number (e.g., "0.0.6-1~noble2" -> "2")
