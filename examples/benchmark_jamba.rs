//! Benchmark for Jamba model using mlxcel-core
//!
//! This binary loads the Jamba hybrid Mamba+Transformer model and benchmarks
//! token generation throughput.

use mlxcel::models::jamba::JambaModel;
use mlxcel_core::generate::{CxxGenerator, SamplingConfig};
use std::time::Instant;

const MODEL_PATH: &str = "models/jamba-3b-4bit";

fn main() {
    println!("=======================================================");
    println!("  Jamba 3B Benchmark - mlxcel-core");
    println!("=======================================================");
    println!();
    ...
    let (model, args) = match JambaModel::load(MODEL_PATH) {
        Ok(result) => result,
        Err(e) => {
    ...
    println!("  Attn layer period: {}", args.attn_layer_period);
    println!("  Attn layer offset: {}", args.attn_layer_offset);
    println!("  Num experts: {:?}", args.num_experts);
    println!();
    ...
    // Test prompt tokens (generic tokens that work with most tokenizers)
